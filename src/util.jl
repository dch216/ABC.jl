...
noop(x::AbstractArray) = return true
...
function cov_weighted(x::AbstractArray{Float64,2}, w::AbstractArray{Float64,1} )
...
function ess(w::AbstractArray{Float64,1})
  sumw = sum(w)
  sumw2 = sum(w.*w)
...
function ess(w::AbstractArray{Float64,1}, repeat::AbstractArray{Int64,1} )
...
function set_distance_scale( plan::abc_pmc_plan_type, theta::AbstractArray{Float64,1}; num_draw::Integer = 40,
